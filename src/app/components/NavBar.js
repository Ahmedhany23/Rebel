...
export default function NavBar() {
  ...
  return (
    <nav className={` justify-between lg:px-0 xl:px-16 flex  bg-[#120B0B] bg-opacity-[51%]  items-center ${hide?'hidden':'fixed'} duration-200 top-0 left-0 right-0  z-10`}>
      ...
      <ul className={`menu sm:flex sm:flex-col lg:hidden gap-6 fixed duration-500 ${open? 'right-0' : 'right-[-100%]'} top-0 bg-[#120B0B]  h-screen pt-6 bg-opacity-[51%]  xl:px-20 lg:pl-24  `}>
      <button className="flex justify-start px-2 text-4xl text-white   hover:text-blue-600 duration-300" onClick={()=>setopen(false)}><IoClose className="hover:animate-spin" /></button>
